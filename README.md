...
Периодическая отправка health JSON в Flat Partner (conf, cron, установщик) — в каталоге [`agent/`](agent/). Там же лежит отдельный `flat_check_agent.sh` — самостоятельный, без установщика скрипт-агент для прямой интеграции в мониторинг (Zabbix external check, systemd timer): установщик из `agent/` его не ставит и не трогает, это другой инструмент — см. раздел 2 в [`agent/README.md`](agent/README.md).
