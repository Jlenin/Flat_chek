...
|---|---|---|
| Файлов с логикой | 3 (два почти-дублирующих монолита + agent-инклюд) | 1 точка входа + 21 модуль в `lib/` |
...
| Что нужно знать, чтобы поправить одну проверку | где именно в 3000-9000 строках | один файл в `lib/`, обычно <900 строк |
...
Подробная карта слоёв, порядок подключения модулей и таблица «что искать в
каком файле» — в [`ARCHITECTURE.md`](ARCHITECTURE.md). Каждый файл в
`lib/**/*.sh` начинается с заголовка (Модуль/Слой/Назначение/Публичные
функции/Зависит от/Не зависит от/Side effects/Источник) — этого обычно
достаточно, чтобы понять файл, не читая соседние.
