...
# _parse_size_to_bytes() НЕ определена здесь: перенесена в lib/core.sh
# (раздел 09_argv, прямо перед usage()/parse_args()) — найдено при финальной
# проверке после консолидации в 3 файла. parse_args() разбирает --chunk-size
# и вызывает эту функцию НЕМЕДЛЕННО, во время разбора флагов — то есть ДО
# того, как точка входа решает, подключать ли lib/logging.sh (решение
# принимается уже ПОСЛЕ parse_args(), по его результату). Раньше это
# работало только потому, что --chunk-size обычно передаётся вместе с -log
# (который сам включает lib/logging.sh), но `./flat_check --chunk-size 50M`
# без -log падал "command not found". Сама функция ничего не знает про
# logging (чистый парсер "50M"/"200000000" в байты), так что перенос в core
# корректен и по смыслу, не только по необходимости.
