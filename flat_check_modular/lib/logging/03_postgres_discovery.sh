...
# Слой: logging
# Назначение: Поиск логов PostgreSQL (путь, формат имени, ротация) отдельно от каталога пакетов FLAT — своя эвристика путей/прав доступа.
# Публичные функции: поиск и валидация директории/файлов логов PostgreSQL
...
# Side effects: читает файловую систему (find/ls), не пишет
#
