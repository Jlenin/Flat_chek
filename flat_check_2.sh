...
#
# JSON/push для дашборда/мониторинга — не здесь, см. agent/flat_check_agent.sh
# (отдельный самостоятельный продукт).
#
...
# --- 3b. Системные метрики (обзор хоста) -----------------------------------
...
_run_selftest_simple() {
    ...
    local pkg_count=${#PKG_PRODUCT[@]}
    [[ "$pkg_count" -ge 100 ]] && _selftest_ok "PKG_PRODUCT entries=$pkg_count" || _selftest_bad "PKG_PRODUCT entries=$pkg_count (want ≥100)"
    if [[ "${PKG_PRODUCT[nginx]:-}" == "Infrastructure" && "${PKG_PRODUCT[fvcs-backend]:-}" == "FVSC" && "${PKG_PRODUCT[fc-frontend]:-}" == "Contact Center" ]]; then
        _selftest_ok "catalog has Infrastructure+FVSC+fc-frontend"
    else
...
Health filters:
  --pkg NAME            single package (health check)
  --product NAME        product filter for health check
                        (in -log mode -p still selects log products)

Examples:
  ./flat_check_2.sh                    # Health check only
  ./flat_check_2.sh -i                 # Interactive wizard
  ./flat_check_2.sh --pkg fss-server
...
JSON/push for dashboard/monitoring: agent/flat_check_agent.sh (separate product).
...
Фильтры проверки:
  --pkg NAME            один пакет (health check)
  --product NAME        фильтр продукта (health check); в -log — выбор лога
...
JSON/push для дашборда/мониторинга: agent/flat_check_agent.sh (отдельный продукт).
EOF
    exit 0
}
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            --scope)
                ...
                ;;
            -p|--product)
                # -log: список продуктов для сбора; иначе — фильтр health check
                if [[ -z "${2:-}" || "$2" == -* ]]; then die "Missing value for $1"; fi
